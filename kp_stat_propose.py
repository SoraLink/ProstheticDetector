...
# ==========================================
# 1. 核心解析逻辑：严格保留 4 类残肢点统计与控制台打印
# ==========================================
def analyze_dataset_stats(file_list, label="Dataset"):
...
def draw_and_save_separately(ld_agg, ld_res, pr_agg, pr_res):
    ...
    # 将图例下移到原标题的位置 (bbox_to_anchor 设为 1.02)
    ...
    print("✅ 执行完毕：已移除图 c 标题，并将图例下放至原标题位置。")
...
if __name__ == "__main__":
    LDP_FILES = [
        ...
        # "./ldpose_annotation/val_final.json",
        "./ldpose_annotation/test_final.json"
    ]
    PRP_FILES = [
        "./propose_annotation/train_final.json",
        # "./propose_annotation/val_final.json",
        "./propose_annotation/test_final.json"
    ]

    print("--- 正在处理 LDPose 数据 ---")
    ld_agg, ld_res = analyze_dataset_stats(LDP_FILES, "LDPose")
    print("--- 正在处理 ProPose 数据 ---")
    pr_agg, pr_res = analyze_dataset_stats(PRP_FILES, "ProPose")
